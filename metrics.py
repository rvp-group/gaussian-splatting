...
# https://github.com/NVlabs/InstantSplat/blob/instantsplat-2dgs/metrics.py
...
from utils.utils_poses.vis_pose_utils import (
    interp_poses_bspline,
    generate_spiral_nerf,
    plot_pose,
)
...
def calculate_depth_metrics(pred_depth, gt_depth):
    ...
    median_gt, median_pred = (
        torch.median(gt_depth_masked),
        torch.median(pred_depth_masked),
    )
    ...
    ratio = torch.max(
        pred_depth_masked / gt_depth_masked, gt_depth_masked / pred_depth_masked
    )
...
def evaluate(args):
    ...
    for scene_dir in args.model_paths:
        ...
        for method in os.listdir(test_dir):
            ...
            # Check if depth directories exist
            gt_depth_dir = Path(args.source_path) / "depths"
            pred_depth_dir = method_dir / "vis"
            has_depth = gt_depth_dir.exists() and pred_depth_dir.exists()

            if has_depth:
                gt_depth_files = sorted(
                    [f.name for f in gt_depth_dir.glob("*_depth.npy")]
                )
                has_depth = len(gt_depth_files) > 0
                if has_depth:
                    print(f"  Found depth data, will evaluate depth metrics")
                else:
                    print(
                        f"  No depth files found in {gt_depth_dir}, skipping depth metrics"
                    )
            else:
                print(f"  Depth directories not found, skipping depth metrics")
            ...
            for idx in tqdm(range(len(renders)), desc="Metric evaluation progress"):
                ...
                out_f.write(
                    f"image name{image_names[idx]}, image idx: {idx}, PSNR: {p.item():.2f}, SSIM: {s:.4f}, LPIPS: {l.item():.4f}\n"
                )
                ssims.append(s)
                psnrs.append(p)
                lpipss.append(l)
                ...
                if has_depth:
                    gt_depth = torch.from_numpy(
                        np.load(gt_depth_dir / gt_depth_files[idx])
                    ).squeeze()
                    pred_depth = torch.from_numpy(
                        tifffile.imread(pred_depth_dir / f"depth_{idx:05d}.tiff")
                    )
            ...
            if has_depth and len(rel_errs) > 0:
                print(
                    "  Rel_err: {:>12.7f}".format(torch.tensor(rel_errs).mean(), ".5")
                )
                ...
                per_view_metrics.update(
                    {
                        "Rel_err": {
                            name: rel_err
                            for rel_err, name in zip(
                                torch.tensor(rel_errs).tolist(), image_names
                            )
                        },
                        "Tau": {
                            name: tau
                            for tau, name in zip(
                                torch.tensor(taus).tolist(), image_names
                            )
                        },
                    }
                )
            ...
            per_view_dict[scene_dir][method].update(
                {
                    ...
                    "Tau": {
                        name: tau
                        for tau, name in zip(torch.tensor(taus).tolist(), image_names)
                    },
                }
            )
            ...
            gt_train_pose, _ = split_train_test(
                pose_colmap, llffhold=8, n_views=args.n_views, verbose=False
            )
            # Apply the same train/test split to optimized poses
            opt_train_pose, _ = split_train_test(
                pose_optimized, llffhold=8, n_views=args.n_views, verbose=False
            )
            # start to align
            pose_optimized = torch.from_numpy(np.array(opt_train_pose))
            poses_gt = torch.from_numpy(np.array(gt_train_pose))
            # align scale first
            trans_gt_align, trans_est_align, _ = align_pose(
                poses_gt[:, :3, -1].numpy(), pose_optimized[:, :3, -1].numpy()
            )
            ...
            with open(pose_path / f"pose_eval.txt", "w") as f:
                f.write(
                    "RPE_t: {:.04f}, RPE_r: {:.04f}, ATE: {:.04f}".format(
                        rpe_trans * 100, rpe_rot * 180 / np.pi, ate
                    )
                )
