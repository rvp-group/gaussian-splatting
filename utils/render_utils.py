...
def save_depthmap(depthmap, path):
    """
    Save depth map as a .png file using 'Spectral_r' colormap visualization.

    Parameters:
    - depthmap: Input depth map data, should be a 2D numpy array
    - path: Path to save the image
    """
    # Get the colormap
    cmap = plt.cm.get_cmap('Spectral_r')

    # Normalize depth data (assuming depth range is [min_depth, max_depth])
    min_depth = np.min(depthmap)
    max_depth = np.max(depthmap)
    depth_normalized = (depthmap - min_depth) / (max_depth - min_depth)

    # Convert depth data to color mapping using colormap
    depth_colored = cmap(depth_normalized)

    # Remove alpha channel from colormap (keep RGB information)
    depth_colored = (depth_colored[:, :, :3] * 255).astype(np.uint8)  # Convert to 8-bit RGB image

    # Save as .png format using PIL
