...
from gaussian_renderer import render2D
...
class GaussianExtractor(object):
    ...
    def reconstruction_optim(
        self, gaussians, viewpoint_stack, optim_test_pose_iter, pipeline, background
    ):
        """
        reconstruct radiance field given cameras
        """
        ...
        for idx, view in tqdm(
            ...
            camera_pose = get_tensor_from_camera(
                view.world_view_transform.transpose(0, 1)
            )
            ...
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                pose_optimizer, T_max=optim_test_pose_iter, eta_min=0.0001
            )
            with tqdm(
    ...
    def extract_mesh_bounded(
        self, voxel_size=0.004, sdf_trunc=0.02, depth_trunc=3, mask_background=True
    ):
        """
    ...
    def export_image(self, path):
        ...
        for idx, viewpoint_cam in tqdm(
            ...
            save_img_u8(
                ...
                os.path.join(
                    render_normal_path, "render_normal_{0:05d}".format(idx) + ".png"
                ),
            )
            save_img_u8(
                self.depth_normals[idx].permute(1, 2, 0).cpu().numpy() * 0.5 + 0.5,
                os.path.join(
                    surf_nomal_path, "surf_normal_{0:05d}".format(idx) + ".png"
                ),
            )
            save_depthmap(
                self.depthmaps[idx][0].cpu().numpy(),
