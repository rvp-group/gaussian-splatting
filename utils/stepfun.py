# taken from https://github.com/NVlabs/InstantSplat/blob/main/utils/stepfun.py
...
def searchsorted(a, v):
    ...
    idx_lo = torch.max(
        torch.where(v_ge_a, i[..., :, None], i[..., :1, None]), -2
    ).values
    idx_hi = torch.min(
        torch.where(~v_ge_a, i[..., :, None], i[..., -1:, None]), -2
    ).values
...
def query(tq, t, y, outside_value=0):
    ...
    yq = torch.where(
        idx_lo == idx_hi,
        torch.full_like(idx_hi, outside_value),
...
def max_dilate_weights(
    t, w, dilation, domain=(-torch.inf, torch.inf), renormalize=False
):
...
def sample_intervals(
    rand, t, w_logits, num_samples, single_jitter=False, domain=(-torch.inf, torch.inf)
):
    ...
    centers = sample(
        rand, t, w_logits, num_samples, single_jitter, deterministic_center=True
    )
...
def weighted_percentile(t, w, ps):
    ...
    fn = lambda cw_i, t_i: math.sorted_interp(
...
def resample(t, tp, vp, use_avg=False):
    ...
    acc0 = torch.cat(
        [torch.zeros(acc.shape[:-1] + (1,), device=acc.device), acc], dim=-1
    )
