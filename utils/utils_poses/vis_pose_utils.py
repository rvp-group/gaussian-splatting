...
# Force a headless backend so metric evaluation works
# in environments without a GUI/Qt platform plugins.
matplotlib.use("Agg")
...
def interp_poses(c2ws, N_views):
    ...
    interp_trans = torch.nn.functional.interpolate(
        trans, size=N_views, mode="linear"
    ).permute(2, 1, 0)
...
def interp_poses_bspline(c2ws, N_novel_imgs, input_times, degree):
    target_trans = torch.tensor(
        scipy_bspline(
            c2ws[:, :3, 3], n=N_novel_imgs, degree=degree, periodic=False
        ).astype(np.float32)
    ).unsqueeze(2)
...
def generate_spiral_nerf(learned_poses, bds, N_novel_views, hwf):
    learned_poses_ = np.concatenate(
        (learned_poses[:, :3, :4].detach().cpu().numpy(), hwf[: len(learned_poses)]),
        axis=-1,
    )
    ...
    c2ws = render_path_spiral(
        c2w_path, up, rads, focal, zdelta, zrate=0.5, rots=N_rots, N=N_novel_views
    )
...
def plot_pose(ref_poses, est_poses, output_path, args, vid=False):
    ...
    if vid:
        for p_idx in range(len(ref_poses)):
            ...
            plt.tight_layout()
            os.makedirs(
                os.path.join(os.path.dirname(output_path), "pose_vid"), exist_ok=True
            )
            pose_vis_path = os.path.join(
                os.path.dirname(output_path),
                "pose_vid",
                "pose_vis_{:03d}.png".format(p_idx),
            )
            print(pose_vis_path)
