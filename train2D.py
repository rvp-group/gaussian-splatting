...
from scene import Scene, SurfelModel
from gaussian_renderer import render, render2D, network_gui
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            if tb_writer is not None:
                ...
                tb_writer.add_scalar(
                    "train_loss_patches/ssim_loss", 1.0 - ssim_value.item(), iteration
                )
                tb_writer.add_scalar(
                    "train_loss_patches/dist_loss", dist_loss.item(), iteration
                )
                tb_writer.add_scalar(
                    "train_loss_patches/normal_loss", normal_loss.item(), iteration
                )
                tb_writer.add_scalar(
                    "train_loss_patches/total_loss", total_loss.item(), iteration
                )
                # EMA smoothed losses for trend visualization
                tb_writer.add_scalar("train_loss_ema/loss", ema_loss_for_log, iteration)
                tb_writer.add_scalar(
                    "train_loss_ema/dist_loss", ema_dist_for_log, iteration
                )
                tb_writer.add_scalar(
                    "train_loss_ema/normal_loss", ema_normal_for_log, iteration
                )
...
def build_args_from_config(data_cf: dict) -> Namespace:
    ...
    args_dict = {
        ...
        "test_iterations": train_cf.get("test_iterations", []),
        "save_iterations": list(train_cf.get("save_iterations", [])),
        ...
        # OptimizationParams
        "iterations": train_cf.get("iterations", 30000),
        ...
        "position_lr_max_steps": train_cf.get("position_lr_max_steps", 30000),
        ...
        "densify_until_iter": train_cf.get("densify_until_iter", 15000),
