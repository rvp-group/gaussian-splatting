...
# https://github.com/NVlabs/InstantSplat/blob/main/scene/gaussian_model.py
...
from simple_knn._C import distCUDA2  # ty:ignore[unresolved-import]
...
class SurfelModel:
    ...
    def create_from_pcd(
        ...
        scale_gaussian: np.ndarray = None,
    ):
    ...
    def training_setup_pp(self, training_args, confidence_lr=None):
        ...
        self.optimizer = PerPointAdam(
            l, lr=0, betas=(0.9, 0.999), eps=1e-15, weight_decay=0.0
        )
    ...
    def load_ply(self, path):
        ...
        if scales.shape[1] != 2:
            print(
                f"[DEBUG PLY] Loaded scales shape: {scales.shape}; expected 2 components for surfel"
            )
            if scales.shape[1] < 2:
                raise ValueError(
                    f"Surfel ply at {path} stores fewer than 2 scale components"
                )
            scales = scales[:, :2]
        ...
        # Filter out points with NaN values in any attribute
        valid_mask = (
            ~np.isnan(xyz).any(axis=1)
            & ~np.isnan(opacities).flatten()
            & ~np.isnan(features_dc).reshape(xyz.shape[0], -1).any(axis=1)
            & ~np.isnan(features_extra).reshape(xyz.shape[0], -1).any(axis=1)
            & ~np.isnan(scales).any(axis=1)
            & ~np.isnan(rots).any(axis=1)
        )
        nan_count = (~valid_mask).sum()
        if nan_count > 0:
            print(
                f"[WARNING] Filtering out {nan_count} points with NaN values ({nan_count / len(valid_mask) * 100:.2f}%)"
            )
            xyz = xyz[valid_mask]
            opacities = opacities[valid_mask]
            features_dc = features_dc[valid_mask]
            features_extra = features_extra[valid_mask]
            scales = scales[valid_mask]
            rots = rots[valid_mask]
            print(f"[INFO] Remaining points after NaN filtering: {len(xyz)}")
