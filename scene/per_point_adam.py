# taken from https://github.com/NVlabs/InstantSplat/blob/main/scene/per_point_adam.py
...
class PerPointAdam(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8, weight_decay=0):
        if not all(0.0 <= x for x in [lr, eps, weight_decay]):
            raise ValueError(
                f"Invalid learning parameters: lr={lr}, eps={eps}, weight_decay={weight_decay}"
            )
        if not all(0.0 <= beta < 1.0 for beta in betas):
            raise ValueError(f"Invalid beta parameters: {betas}")

        defaults = dict(
            lr=lr, betas=betas, eps=eps, weight_decay=weight_decay, per_point_lr=None
        )
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                exp_avg.masked_scatter_(
                    mask, exp_avg[mask].mul_(beta1).add_(grad[mask], alpha=1 - beta1)
                )
                ...
                if per_point_lr is not None:
                    ...
                    if per_point_lr.device != p.data.device:
                        raise ValueError(
                            "per_point_lr must be on the same device as parameter"
                        )

                    expected_shape = p.data.shape[:1] + (1,) * (p.data.dim() - 1)
                    if per_point_lr.shape != expected_shape:
                        raise ValueError(
                            f"Invalid per_point_lr shape. Expected {expected_shape}, got {per_point_lr.shape}"
                        )
