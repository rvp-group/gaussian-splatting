...
def loadCameras(poses, viewpoint_stack):
    ...
    # load interpolated poses
        ...
        viewpoint_stack = [copy.deepcopy(vp) for vp in viewpoint_stack * repeat_times][
            : poses.shape[0]
        ]
        for idx in range(poses.shape[0]):
            ...
            viewpoint_stack[idx].world_view_transform = (
                torch.tensor(getWorld2View2(R, T)).transpose(0, 1).cuda()
            )
            viewpoint_stack[idx].full_proj_transform = (
                viewpoint_stack[idx]
                .world_view_transform.unsqueeze(0)
                .bmm(viewpoint_stack[idx].projection_matrix.unsqueeze(0))
            ).squeeze(0)
            viewpoint_stack[idx].camera_center = viewpoint_stack[
                idx
